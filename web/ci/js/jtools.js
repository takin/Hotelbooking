...
function getCitiesArrayIndex(search_value, cities_array)
{
    ...
    {
        if (cities_array[countryIndex][0] === search_value)
            return countryIndex;
    }
...
function cardchange(base_url)
{
    ...
    if ((cccurrency.toLowerCase() != "all") && (cccurrency.toLowerCase() != settleCurrency.value.toLowerCase()))
    {
        ...
        while ((i < settleCurrency.options.length) && (settleCurrency.options[i].value.toLowerCase() != cccurrency.toLowerCase())) {
            i++;
        }
...
function auto_persons_count(source_field_id, target_field_id, max_persons)
{
    ...
    var i = 0;
    while ((i < selectBox.options.length) && (selectBox.options[i].value != targetValue)) {
        i++;
    }
