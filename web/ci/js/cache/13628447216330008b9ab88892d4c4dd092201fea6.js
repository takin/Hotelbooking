...
{var arrShownHostelTitles=[];$('#property_list').children('div :visible').each(function(){arrShownHostelTitles.push($(this).find(".hostel_title").text());});var that=this;var image=new google.maps.MarkerImage("http://"+window.location.host+'/images/map-marker.png',new google.maps.Size(28,28),new google.maps.Point(0,0),new google.maps.Point(0,29));for(var i in this.markers){var markerTitle=$.trim(this.markers[i].title);var markers_lat=this.markers[i].lat;var markers_lng=this.markers[i].lng;var markers_content=this.markers[i].content;var markers_gmarker=this.markers[i].gmarker;var gbounds=this.gbounds;var custom_content=this.custom_content;var openInfoWindow=this.openInfoWindow;$.each(arrShownHostelTitles,function(key,value){if($.trim(value)==markerTitle){var gmarker=new google.maps.Marker({position:new google.maps.LatLng(markers_lat,markers_lng),map:window.gmap,title:markerTitle,icon:image,custom_content:markers_content});markers_gmarker=gmarker;google.maps.event.addListener(gmarker,'click',function(){openInfoWindow(that,custom_content);});gbounds.extend(gmarker.position);}});}};GoogleMap.prototype.removeMap=function()
