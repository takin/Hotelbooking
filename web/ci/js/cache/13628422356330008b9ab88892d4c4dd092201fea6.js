...
{var that=this;for(var i in this.markers){var image=new google.maps.MarkerImage("http://"+window.location.host+'/images/map-marker.png',new google.maps.Size(28,28),new google.maps.Point(0,0),new google.maps.Point(0,29));var markerTitle=$.trim(this.markers[i].title);$('#property_list').children('div :visible').each(function(){console.info(this);alert("here");var hostel_title=$(this).find(".hostel_title").text();alert("$.trim(hostel_title)=="+$.trim(hostel_title)+":::::markerTitle=="+markerTitle+":::::");if($.trim(hostel_title)==markerTitle){var gmarker=new google.maps.Marker({position:new google.maps.LatLng(this.markers[i].lat,this.markers[i].lng),map:window.gmap,title:this.markers[i].title,icon:image,custom_content:this.markers[i].content});this.markers[i].gmarker=gmarker;google.maps.event.addListener(gmarker,'click',function(){that.openInfoWindow(this,this.custom_content);});this.gbounds.extend(gmarker.position);}});}};GoogleMap.prototype.removeMap=function()
