...
{this.clearMarkers();window.gbounds=null;};GoogleMap.prototype.clearMarkers=function()
{window.markers=Array();this.marker_id_to_focus=-1;};GoogleMap.prototype.drawMarkers=function()
{var that=this;var arrShownHostelTitles=[];$('#property_list').children('div :visible').each(function(){arrShownHostelTitles.push($(this).find(".hostel_title").text());});var image=new google.maps.MarkerImage("http://"+window.location.host+'/images/map-marker.png',new google.maps.Size(28,28),new google.maps.Point(0,0),new google.maps.Point(0,29));for(var i in window.markers){var markerTitle=$.trim(window.markers[i].title);for(var j in arrShownHostelTitles){if($.trim(arrShownHostelTitles[j])===markerTitle)
{console.log($.trim(arrShownHostelTitles[j]),markerTitle);var gmarker=new google.maps.Marker({position:new google.maps.LatLng(window.markers[i].lat,window.markers[i].lng),map:window.gmap,title:window.markers[i].title,icon:image,custom_content:window.markers[i].content});window.markers[i].gmarker=gmarker;google.maps.event.addListener(gmarker,'click',function(){that.openInfoWindow(this,this.custom_content);});window.gbounds.extend(gmarker.position);}}}};GoogleMap.prototype.redrawMarkers=function()
{this.clearMarkers();var that=this;var arrShownHostelTitles=$('#property_list').children('div :visible');var image=new google.maps.MarkerImage("http://"+window.location.host+'/images/map-marker.png',new google.maps.Size(28,28),new google.maps.Point(0,0),new google.maps.Point(0,29));for(var i in arrShownHostelTitles){alert('$(arrShownHostelTitles[i]).find(".hostel_title").text()='+$(arrShownHostelTitles[i]).find(".hostel_title").text());alert('$(arrShownHostelTitles[i]).find(".input_geo_latitude").val()='+$(arrShownHostelTitles[i]).find(".input_geo_latitude").val());alert('$(arrShownHostelTitles[i]).find(".input_geo_longitude").val()='+$(arrShownHostelTitles[i]).find(".input_geo_longitude").val());var gmarker=new google.maps.Marker({position:new google.maps.LatLng(window.markers[i].lat,window.markers[i].lng),map:window.gmap,title:$(arrShownHostelTitles[i]).find(".hostel_title").text(),icon:image,custom_content:window.markers[i].content});window.markers[i].gmarker=gmarker;google.maps.event.addListener(gmarker,'click',function(){that.openInfoWindow(this,this.custom_content);});window.gbounds.extend(gmarker.position);}};GoogleMap.prototype.removeMap=function()
...
var point=landmark_LatLng.split("###");var lat=point[0];var Lng=point[1];var citymap={center:new google.maps.LatLng(lat,Lng)};var LandmarkOptions={strokeColor:"#4E89C9",strokeOpacity:0.8,strokeWeight:2,fillColor:"#4E89C9",fillOpacity:0.35,map:window.gmap,center:citymap.center,radius:2000};window.cityCircle=new google.maps.Circle(LandmarkOptions);};GoogleMap.prototype.changeMarkerIcon=function(pDiv){var hostel_title=$.trim($(pDiv).find(".hostel_title").text());if(window.markers.length!=0){for(var i in window.markers){if(window.markers[i].gmarker!=null)
{if(hostel_title==$.trim(window.markers[i].gmarker.getTitle()))
{var image=new google.maps.MarkerImage("http://"+window.location.host+'/images/marker_selected.png',new google.maps.Size(28,28),new google.maps.Point(0,0),new google.maps.Point(0,29));window.markers[i].gmarker.setIcon(image);}}}}};GoogleMap.prototype.setMarkerIconToOriginal=function(pDiv){var hostel_title=$.trim($(pDiv).find(".hostel_title").text());if(window.markers.length!=0){for(var i in window.markers){if(window.markers[i].gmarker!=null)
{if(hostel_title==$.trim(window.markers[i].gmarker.getTitle()))
{var image=new google.maps.MarkerImage("http://"+window.location.host+'/images/map-marker.png',new google.maps.Size(28,28),new google.maps.Point(0,0),new google.maps.Point(0,29));window.markers[i].gmarker.setIcon(image);}}}}};
