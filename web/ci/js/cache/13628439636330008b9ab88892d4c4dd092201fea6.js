...
{var arrShownHostelTitles=[];$('#property_list').children('div :visible').each(function(){arrShownHostelTitles.push($(this).find(".hostel_title").text());});var that=this;var image=new google.maps.MarkerImage("http://"+window.location.host+'/images/map-marker.png',new google.maps.Size(28,28),new google.maps.Point(0,0),new google.maps.Point(0,29));for(var i in this.markers){var markerTitle=$.trim(GoogleMap.markers[i].title);$.each(arrShownHostelTitles,function(key,value){if($.trim(value)==markerTitle){var gmarker=new google.maps.Marker({position:new google.maps.LatLng(GoogleMap.markers[i].lat,GoogleMap.markers[i].lng),map:window.gmap,title:GoogleMap.markers[i].title,icon:image,custom_content:GoogleMap.markers[i].content});GoogleMap.markers[i].gmarker=gmarker;google.maps.event.addListener(gmarker,'click',function(){GoogleMap.openInfoWindow(GoogleMap,GoogleMap.custom_content);});GoogleMap.gbounds.extend(gmarker.position);}});}};GoogleMap.prototype.removeMap=function()
