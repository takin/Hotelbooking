...
{this.clearMarkers();alert("this.clearMarkers()");var that=this;var arrShownHostelTitles=$('#property_list').children('div :visible');var image=new google.maps.MarkerImage("http://"+window.location.host+'/images/map-marker.png',new google.maps.Size(28,28),new google.maps.Point(0,0),new google.maps.Point(0,29));for(var i in arrShownHostelTitles){var title=$.trim($(arrShownHostelTitles[i]).find(".hostel_title").text());alert('title='+title);var gmarker=new google.maps.Marker({position:new google.maps.LatLng($(arrShownHostelTitles[i]).find(".input_geo_latitude").val(),$(arrShownHostelTitles[i]).find(".input_geo_longitude").val()),map:window.gmap,title:title,icon:image,custom_content:title});window.gbounds.extend(gmarker.position);}};GoogleMap.prototype.removeMap=function()
