...
{var that=this;for(var i in this.markers){var content=this.markers[i].content;console.log($(content+" .alignleft"));console.log($("#property_list > div:eq("+i+") .hostel_title").text());var image=new google.maps.MarkerImage("http://"+window.location.host+'/images/map-marker.png',new google.maps.Size(28,28),new google.maps.Point(0,0),new google.maps.Point(0,29));var gmarker=new google.maps.Marker({position:new google.maps.LatLng(this.markers[i].lat,this.markers[i].lng),map:window.gmap,title:this.markers[i].title,icon:image,custom_content:this.markers[i].content});this.markers[i].gmarker=gmarker;google.maps.event.addListener(gmarker,'click',function(){that.openInfoWindow(this,this.custom_content);});this.gbounds.extend(gmarker.position);}
console.info(this);};GoogleMap.prototype.removeMap=function()
