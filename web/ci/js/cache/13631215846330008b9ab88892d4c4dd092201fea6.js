...
{var that=this;var arrShownHostelTitles=[];$('#property_list').children('div :visible').each(function(){arrShownHostelTitles.push($(this).find(".hostel_title").text());});var arrShownHostels=$('#property_list').children('div :visible');console.info(arrShownHostels);alert("arrShownHostels");var image=null
image=new google.maps.MarkerImage("http://"+window.location.host+'/images/map-marker.png',new google.maps.Size(28,28),new google.maps.Point(0,0),new google.maps.Point(0,29));for(var i in window.markers){var markerTitle=$.trim(window.markers[i].title);for(var j in arrShownHostelTitles){if($.trim(arrShownHostelTitles[j])==markerTitle)
{console.log($.trim(arrShownHostelTitles[j]),markerTitle);var gmarker=new google.maps.Marker({position:new google.maps.LatLng(window.markers[i].lat,window.markers[i].lng),map:window.gmap,title:window.markers[i].title,icon:image,custom_content:window.markers[i].content});window.markers[i].gmarker=gmarker;google.maps.event.addListener(gmarker,'click',function(){that.openInfoWindow(this,this.custom_content);});window.gbounds.extend(gmarker.position);}}}};GoogleMap.prototype.removeMap=function()
