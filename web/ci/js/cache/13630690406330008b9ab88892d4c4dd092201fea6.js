...
var point=landmark_LatLng.split("###");var lat=point[0];var Lng=point[1];var citymap={center:new google.maps.LatLng(lat,Lng)};var LandmarkOptions={strokeColor:"#4E89C9",strokeOpacity:0.8,strokeWeight:2,fillColor:"#4E89C9",fillOpacity:0.35,map:window.gmap,center:citymap.center,radius:2000};window.cityCircle=new google.maps.Circle(LandmarkOptions);};GoogleMap.prototype.changeMarkerIcon=function(pDiv){var hostel_title=$.trim($(pDiv).find(".hostel_title").text());console.log(hostel_title);console.log(window.markers[0].gmarker.getTitle());console.info(window.markers.length);if(window.markers.length!=0){for(var i in window.markers){if(hostel_title==$.trim(window.markers[i].gmarker.getTitle()))
{console.info(window.markers[i]);var image=new google.maps.MarkerImage("http://"+window.location.host+'/images/marker_selected.png',new google.maps.Size(28,28),new google.maps.Point(0,0),new google.maps.Point(0,29));window.markers[i].gmarker.setIcon(image);}}}};
