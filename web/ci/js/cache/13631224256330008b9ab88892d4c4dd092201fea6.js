...
{this.clearMarkers();var that=this;var arrShownHostelTitles=$('#property_list').children('div :visible');var image=new google.maps.MarkerImage("http://"+window.location.host+'/images/map-marker.png',new google.maps.Size(28,28),new google.maps.Point(0,0),new google.maps.Point(0,29));for(var j in arrShownHostelTitles){alert('$(arrShownHostelTitles[j]).find(".hostel_title").text()='+$(arrShownHostelTitles[j]).find(".hostel_title").text());var gmarker=new google.maps.Marker({position:new google.maps.LatLng(window.markers[i].lat,window.markers[i].lng),map:window.gmap,title:window.markers[i].title,icon:image,custom_content:window.markers[i].content});window.markers[i].gmarker=gmarker;google.maps.event.addListener(gmarker,'click',function(){that.openInfoWindow(this,this.custom_content);});window.gbounds.extend(gmarker.position);}};GoogleMap.prototype.removeMap=function()
