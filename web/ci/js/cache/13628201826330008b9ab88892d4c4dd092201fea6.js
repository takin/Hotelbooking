...
return false;});initpaging(result_per_page);};PWebFilterApp.prototype.refresh=function(more_results){more_results=more_results||0;this.results_limit=this.results_limit+more_results;this.sort_hits(this.actual_sort_index.row,this.actual_sort_order,true);initpaging(result_per_page);};PWebFilterApp.prototype.toggleMap=function(map_slug){alert("map_slug="+map_slug+"::::");this.pweb_maps[map_slug].toggle();if(this.pweb_maps[map_slug].isMapEnable()===true)
...
{data=jQuery.parseJSON(data);pweb_filter.setRequestData(data.request);pweb_filter.setData(data.property_list);totalRecords=data.property_list.length;pweb_filter.addFilterMap('city','city_map_container','en',data.city_info.city_geo_lat,data.city_info.city_geo_lng);pweb_filter.addFilterMap('city_side_map','city_side_map_container','en',data.city_info.city_geo_lat,data.city_info.city_geo_lng);pweb_filter.addFilterMap('property',"will_set_on_tab_click",'en',data.city_info.city_geo_lat,data.city_info.city_geo_lng);pweb_filter.setClickSort('data_sort_controls','sortname-tous','propertyName');pweb_filter.setClickSort('data_sort_controls','sortprice-tous','display_price');pweb_filter.setClickSort('data_sort_controls','sortcote-tous','overall_rating');$('#data_sort_controls').show();pweb_filter.apply_filters();pweb_filter.set_init_filters_value();pweb_filter.init_action_filters();$('#show_more_results').click(function()
...
{pweb_filter.toggleMap('city');pweb_filter.toggleMap('city_side_map');$(this).hide();$('#map_button_side').hide();$('#city_map_hide').show();return false;});$('#city_map_hide').click(function()
