...
{$('#'+id).attr('checked',this.FiltersInitValues[id]);}};PWebFilterApp.prototype.reset_Pricefilters=function(){var that=this;this.PriceFilterMin=this.PriceRangeMin;this.PriceFilterMax=this.PriceRangeMax;$("#slider_price").slider({change:null});$("#slider_price").slider({values:[that.PriceRangeMin,that.PriceRangeMax]});document.getElementById('filter_price').innerHTML=that.PriceCurrencySymbol+$("#slider_price").slider("values",0)+" - "+that.PriceCurrencySymbol+$("#slider_price").slider("values",1);$("#slider_price").slider({change:function(event,ui){that.change_price_filter(event,ui);}});};PWebFilterApp.prototype.addFilterMap=function(map_slug,city_map_div_id,map_lang,lat,lng){this.pweb_maps[map_slug]=new PWebFilterMap(city_map_div_id,map_lang,lat,lng);};PWebFilterApp.prototype.apply_filters=function(){this.results_limit=20;this.$data_empty_msg.hide();this.$sort_controls_div.hide();this.$data_div.html("");this.$data_loading_msg.show();$('#show_more_results').hide();this.init_counts();this.jtable_hits=this.jtable.filter(this.get_filters());if(this.count_st==0){this.compute_counts();this.update_counts();this.count_st++;}
this.update_counts();this.sort_hits(this.actual_sort_index.row,this.actual_sort_order);this.update();this.updateMap();};PWebFilterApp.prototype.updateMap=function(){this.pweb_maps['property'].prop_number_to_focus=-1;if(this.pweb_maps['city'].enabled===true)
...
{this.FiltersCounts['facility-count-'+current_facility_id]++;}}}}};PWebFilterApp.prototype.update_counts=function(){if($('#applied_filter_hosting_price').css('display')=='none'&&$('#applied_filter_hosting_rating').css('display')=='none'&&$('#applied_filter_hosting_property').css('display')=='none'&&$('#applied_filter_hosting_facilities').css('display')=='none'&&$('#applied_filter_hosting_districts').css('display')=='none'&&$('#applied_filter_hosting_landmarks').css('display')=='none'){$('#filters_text').hide();}else{$('#filters_text').show();}
...
{if(that.count_st==0)
that.FiltersCounts['prop-types-count-5']++;return true;}}};};PWebFilterApp.prototype.setData=function(json_data){jOrder.logging=null;this.jtable=jOrder(json_data).index('propertyNumber',['propertyNumber'],{grouped:false,ordered:true,type:jOrder.number}).index('propertyType',['propertyType'],{grouped:true,ordered:true,type:jOrder.string});this.FiltersCounts['city_results_count_total']=json_data.length;};PWebFilterApp.prototype.setRequestData=function(json_request_data){this.request=json_request_data;this.PriceCurrencySymbol=this.request.display_currency;};PWebFilterApp.prototype.change_price_filter=function(event,ui){this.PriceFilterMin=ui.values[0];this.PriceFilterMax=ui.values[1];if((pweb_filter.PriceRangeMin==this.PriceFilterMin&&pweb_filter.PriceRangeMax==this.PriceFilterMax))
{$('#applied_filter_hosting_price').hide();}else{$('#applied_filter_hosting_price').show();}
this.apply_filters();};PWebFilterApp.prototype.change_rating_filter=function(event,ui){this.RatingFilterMin=ui.values[0];this.RatingFilterMax=ui.values[1];if((pweb_filter.RatingRangeMin==this.RatingFilterMin&&pweb_filter.RatingRangeMax==this.RatingFilterMax))
{$('#applied_filter_hosting_rating').hide();}else{$('#applied_filter_hosting_rating').show();}
this.apply_filters();};PWebFilterApp.prototype.init_action_filters=function(){var that=this;this.PriceRangeMin=Math.floor(this.PriceRangeMin);this.PriceRangeMax=Math.ceil(this.PriceRangeMax);$("#slider_price").slider({range:true,min:that.PriceRangeMin,max:that.PriceRangeMax,values:[that.PriceRangeMin,that.PriceRangeMax],slide:function(event,ui){document.getElementById('filter_price').innerHTML=that.PriceCurrencySymbol+ui.values[0]+" - "+that.PriceCurrencySymbol+ui.values[1];},change:function(event,ui){that.change_price_filter(event,ui);}});document.getElementById('filter_price').innerHTML=that.PriceCurrencySymbol+$("#slider_price").slider("values",0)+" - "+that.PriceCurrencySymbol+$("#slider_price").slider("values",1);$("#slider_rating").slider({range:true,min:that.RatingRangeMin,max:that.RatingRangeMax,values:[that.RatingRangeMin,that.RatingRangeMax],slide:function(event,ui){document.getElementById('filter_rating').innerHTML=ui.values[0]+"% - "+ui.values[1]+"%";that.RatingFilterMin=ui.values[0];that.RatingFilterMax=ui.values[1];},change:function(event,ui){that.change_rating_filter(event,ui);}});document.getElementById('filter_rating').innerHTML=$("#slider_rating").slider("values",0)+"% - "+$("#slider_rating").slider("values",1)+"%";$('#breakfast_2nd_filter').click(function()
...
alert('$("#divLandmark_"+property_number+" input:radio:first").length='+$("#divLandmark_"+property_number+" input:radio:first").length);if($("#divLandmark_"+property_number+" input:radio:first").length>0)
...
{var landmarklatlng=$("#divLandmark_"+property_number+" input:radio:checked").val();this.changeLandmarkLayer(landmarklatlng);}
else
...
var point=landmark_LatLng.split("###");var lat=point[0];var Lng=point[1];var citymap={center:new google.maps.LatLng(lat,Lng)};var LandmarkOptions={strokeColor:"#FF0000",strokeOpacity:0.8,strokeWeight:2,fillColor:"#FF0000",fillOpacity:0.35,map:map,center:citymap.center,radius:2000};window.cityCircle=new google.maps.Circle(LandmarkOptions);}
