...
{pweb_filter.refresh(10);return false;});pweb_filter.toggleMap('city');$('#reset_filters').click(function()
...
{window.gmap.setZoom(14);window.gmap.setCenter(window.markers[this.marker_id_to_focus].gmarker.position);this.openInfoWindow(window.markers[this.marker_id_to_focus].gmarker,window.markers[this.marker_id_to_focus].gmarker.custom_content);}};GoogleMap.prototype.drawMap=function()
{that=this;var script_id="google_map_api_script";if($("#"+script_id).length<=0)
{var script=document.createElement("script");script.type="text/javascript";script.id=script_id;script.src="https://maps.google.com/maps/api/js?sensor=false&language="+this.map_lang+"&callback=gmap_start";window.gmap_start=function(){that.init();};document.body.appendChild(script);}
else
{this.init();}};GoogleMap.prototype.addMarker=function(index,lat,lng,title,content)
{var marker={title:title,lat:lat,lng:lng,content:content,gmarker:null};window.markers[index]=marker;};GoogleMap.prototype.clearMap=function()
{this.clearMarkers();this.gbounds=null;};GoogleMap.prototype.clearMarkers=function()
{window.markers=Array();this.marker_id_to_focus=-1;if(window.gmarkers){for(i in window.gmarkers){window.gmarkers[i].setMap(null);}
window.gmarkers.length=0;}};GoogleMap.prototype.drawMarkers=function()
{this.fillMakersArray();this.addMarkersToMap();};GoogleMap.prototype.getItemsInPage=function()
{var that=this;var show_per_page=parseInt($('#show_per_page').val());var page_num=parseInt($('#page_navigation .active_page').attr("longdesc"));var start_from=page_num*show_per_page;var end_on=start_from+show_per_page;return $('#property_list').children().slice(start_from,end_on);};GoogleMap.prototype.fillMakersArray=function()
{var that=this;that.clearMap();var property_list=that.getItemsInPage();$.each(property_list,function(index,value){var property_number=$(value).attr("rel");$("#city_map_view_"+property_number).html("")
GoogleMap.prototype.addMarker(index,$("#input_geo_latitude_"+property_number).val(),$("#input_geo_longitude_"+property_number).val(),$.trim($("#hostel_title_"+property_number).text()),$.trim($("#map_InfoWindow_"+property_number).html()),property_number);});return window.markers;};GoogleMap.prototype.addMarkersToMap=function()
{if(window.markers.length<1)
{window.markers=this.fillMakersArray();}
var that=this;var image=new google.maps.MarkerImage("http://"+window.location.host+'/images/map-marker.png',new google.maps.Size(28,28),new google.maps.Point(0,0),new google.maps.Point(0,29));if(this.gbounds===null)
{this.gbounds=new google.maps.LatLngBounds();}
for(var i in window.markers){window.gmarkers[i]=new google.maps.Marker({position:new google.maps.LatLng(window.markers[i].lat,window.markers[i].lng),map:window.gmap,title:window.markers[i].title,icon:image,custom_content:window.markers[i].content});window.markers[i].gmarker=window.gmarkers[i];google.maps.event.addListener(window.gmarkers[i],'click',function(){that.openInfoWindow(this,this.custom_content);});google.maps.event.addListener(window.gmarkers[i],'mouseover',function(){var image_selected=new google.maps.MarkerImage("http://"+window.location.host+'/images/map-marker_selected.png',new google.maps.Size(28,28),new google.maps.Point(0,0),new google.maps.Point(0,29));this.setIcon(image_selected);this.setZIndex(100000);that.changeHostelBackground(this,"mouseover");});google.maps.event.addListener(window.gmarkers[i],'mouseout',function(){this.setIcon(image);this.setZIndex(0);that.changeHostelBackground(this,"mouseout");});this.gbounds.extend(window.gmarkers[i].position);}};GoogleMap.prototype.removeMap=function()
...
var point=landmark_LatLng.split("###");var lat=point[0];var Lng=point[1];var citymap={center:new google.maps.LatLng(lat,Lng)};var LandmarkOptions={strokeColor:"#4E89C9",strokeOpacity:0.8,strokeWeight:2,fillColor:"#4E89C9",fillOpacity:0.35,map:window.gmap,center:citymap.center,radius:2000};window.cityCircle=new google.maps.Circle(LandmarkOptions);};GoogleMap.prototype.changeMarkerIconToSelected=function(pDiv){this.changeMarkerIcon(pDiv,"selected");};GoogleMap.prototype.setMarkerIconToOriginal=function(pDiv){this.changeMarkerIcon(pDiv,"original");};GoogleMap.prototype.changeMarkerIcon=function(pDiv,pIconType){var property_number=$(pDiv).attr("rel");var hostel_title=$.trim($("#hostel_title_"+property_number).text());var image=null;if(pIconType==="selected")
{image="map-marker_selected.png";}
else
{image="map-marker.png";}
$("#city_info_"+property_number).removeClass('property_info_hover');if(pIconType==="selected")
{$("#city_info_"+property_number).addClass('property_info_hover');}
if(window.markers.length!==0){for(var i in window.markers){if(window.markers[i].gmarker!==null)
{if(window.markers[i].gmarker.getZIndex()===100000){window.markers[i].gmarker.setZIndex(0);}
if(hostel_title===$.trim(window.markers[i].gmarker.getTitle()))
{var image=new google.maps.MarkerImage("http://"+window.location.host+'/images/'+image,new google.maps.Size(28,28),new google.maps.Point(0,0),new google.maps.Point(0,29));window.markers[i].gmarker.setZIndex(100000);window.markers[i].gmarker.setIcon(image);}}}}};GoogleMap.prototype.changeHostelBackground=function(pMarker,pDivEventToTrigger){var that=this;var property_list=that.getItemsInPage();$.each(property_list,function(index,value){if($.trim($(value).find(".hostel_title").text())===pMarker.getTitle())
