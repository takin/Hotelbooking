...
{var arrShownHostelTitles=[];$('#property_list').children('div :visible').each(function(){arrShownHostelTitles.push($(this).find(".hostel_title").text());});var that=this;var image=new google.maps.MarkerImage("http://"+window.location.host+'/images/map-marker.png',new google.maps.Size(28,28),new google.maps.Point(0,0),new google.maps.Point(0,29));for(var i in this.markers){var markerTitle=$.trim(this.markers[i].title);$.each(arrShownHostelTitles,function(key,value){if($.trim(value)==markerTitle){var gmarker=new google.maps.Marker({position:new google.maps.LatLng(that.markers[i].lat,that.markers[i].lng),map:window.gmap,title:that.markers[i].title,icon:image,custom_content:that.markers[i].content});that.markers[i].gmarker=gmarker;google.maps.event.addListener(gmarker,'click',function(){that.openInfoWindow(that,that.custom_content);});that.gbounds.extend(gmarker.position);}});}};GoogleMap.prototype.removeMap=function()
