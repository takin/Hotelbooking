...
{this.clearMarkers();var that=this;var arrShownHostelTitles=$('#property_list').children('div :visible');var image=new google.maps.MarkerImage("http://"+window.location.host+'/images/map-marker.png',new google.maps.Size(28,28),new google.maps.Point(0,0),new google.maps.Point(0,29));for(var i in arrShownHostelTitles){var gmarker=new google.maps.Marker({position:new google.maps.LatLng($(arrShownHostelTitles[i]).find(".input_geo_latitude").val(),$(arrShownHostelTitles[i]).find(".input_geo_longitude").val()),map:window.gmap,title:$(arrShownHostelTitles[i]).find(".hostel_title").text(),icon:image,custom_content:$(arrShownHostelTitles[i]).find(".hostel_title").text()});window.markers[i].gmarker=gmarker;google.maps.event.addListener(gmarker,'click',function(){that.openInfoWindow(this,this.custom_content);});window.gbounds.extend(gmarker.position);}};GoogleMap.prototype.removeMap=function()
