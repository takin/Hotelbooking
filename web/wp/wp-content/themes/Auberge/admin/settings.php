...
$options = array(
    ...
    array("name" => "HB API specific options",
        "type" => "title"),
    array("name" => "Charge Booking Fees on HB API",
        "desc" => "Please choose weather to charge Booking Fees on HB",
        "id" => $shortname . "_hb_charge_booking_fees",
        "std" => "",
        "type" => "selectplus",
        "options" => array(array("Disable", "false"), array("Enable", "true"))),
    array("name" => "Import local HW reviews to HB",
        "desc" => "Please choose weather to import local HW reviews to HB",
        "id" => $shortname . "_hb_import_hw_reviews",
        "std" => "",
        "type" => "selectplus",
        "options" => array(array("Disable", "false"), array("Enable", "true"))),
        ...
        switch ($value['type']) {
            ...
            case 'textarea':
                ?>
                    ...
                    echo get_settings(stripslashes($value['id']));
                } else {
            ...
            case 'select':
                ?>
                        <tr>
                            <td valign="top" width="20%" rowspan="2"><?php echo $value['name']; ?></td>
                            <td width="80%"><select style="width:240px;" name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>"><?php foreach ($value['options'] as $option) { ?><option<?php if (get_settings($value['id']) == $option) {
                        ...
                        <tr>
                            <td valign="top" width="20%" rowspan="2"><?php echo $value['name']; ?></td>
                            <td width="80%"><select style="width:240px;" name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>"><?php foreach ($value['options'] as $option) { ?><option<?php if (get_settings($value['id']) == $option[1]) {
                        echo ' selected="selected"';
                    } elseif ($option[1] == $value['std']) {
                        echo ' selected="selected"';
                    } ?> value="<?php echo $option[1]; ?>"><?php echo $option[0]; ?></option><?php } ?></select></td>
                        </tr>
