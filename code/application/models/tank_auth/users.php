...
class Users extends CI_Model
{
   ...
   */
	function set_profile_data($user_id,$data)
	{
	  if(!is_null($user_id))
	  {
	    $query = $this->db->get_where(self::TABLE_PROFILE, array('user_id' => $user_id));
	    if($query->num_rows() == 0)
	    {
	      $this->create_profile($user_id);
	    }
      ...
      return $this->db->update(self::TABLE_PROFILE, $data); 	    
	  }
	...
	{
	  $domain_ID = $this->Db_links->get_domain_ID($_SERVER['HTTP_HOST']);
	  
		$this->db->set('user_id', $user_id);
		$this->db->set('site_domain_registered_id', $domain_ID);
		$this->db->set('favorite_lang_id', $domain_ID);
