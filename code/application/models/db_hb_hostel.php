...
class Db_hb_hostel extends CI_Model {
	...
	function get_location_properties($country_system_name, $city_system_name, $language_code = "en", $strCurrencyCode = "GBP", $limit = 0, $filters = array()) {
		...
		FROM hb_hostel h
	  	...
	  	LEFT JOIN hb_hostel_description hd_tr ON h_tr.property_number = hd_tr.hostel_hb_id AND hd_tr.`language` = '$language_code'
	  	$landmark_join
      	$district_join
		...
		if (!ISDEVELOPMENT) {
			$sql .= "AND h.last_update > (SELECT DATE_FORMAT(MAX(last_update), '%Y-%m-%d') FROM hb_hostel)";
		}
	...
	public function update_hostel_from_array(array $hostel) {
		...
		if (empty($propertyNumber)) {
			throw new Exception(sprintf("Error updating hostel named %s: no property number", $hostel["property_name"]));
		}
		...
		if (!$isUpdated) {
			throw new Exception(sprintf("Hostel with property number %s unable to be updated.", $propertyNumber));
		}
	}
	...
	public function delete_all_images_for_property($propertyNumber) {
		$isDeleted = $this -> CI -> db -> delete(self::HOSTEL_IMAGE_TABLE, array('hostel_hb_id' => $propertyNumber));

		if (!$isDeleted) {
			throw new Exception(sprintf("Error occurred while deleting images for hb hostel with property number %s", $propertyNumber));
		}
	}

	public function delete_all_extras_for_property($propertyNumber) {
		$isDeleted = $this -> CI -> db -> delete(self::HOSTEL_EXTRA_TABLE, array('hostel_hb_id' => $propertyNumber));

		if (!$isDeleted) {
			throw new Exception(sprintf("Error occurred while deleting extras for hb hostel with property number %s", $propertyNumber));
		}
	}

	public function delete_all_facilities_for_property($propertyNumber) {
		$isDeleted = $this -> CI -> db -> delete(self::HOSTEL_FEATURE_TABLE, array('hostel_hb_id' => $propertyNumber));

		if (!$isDeleted) {
			throw new Exception(sprintf("Error occurred while deleting facilities/features for hb hostel with property number %s", $propertyNumber));
		}
	}

	public function delete_all_prices_for_property($propertyNumber) {
		$isDeleted = $this -> CI -> db -> delete(self::HOSTEL_PRICE_TABLE, array('hostel_hb_id' => $propertyNumber));

		if (!$isDeleted) {
			throw new Exception(sprintf("Error occurred while deleting prices for hb hostel with property number %s", $propertyNumber));
		}
	}
	...
	function update_hostel_booking_info($property_number, $email = NULL, $post_code = NULL, $phone = NULL, $fax = NULL, $currency_code = NULL) {
	...
	public function insert_or_update_hb_hostel_data(array $hostelData) {
		...
		if (!$this -> does_city_exist($cityId))
			return NULL;
	...
	private function insert_hb_hostel_data(array $hostelData) {
		...
		if (isset($hostel["hb_hostel_id"]))
			unset($hostel["hb_hostel_id"]);
	...
	function insert_hostel_from_array(array $hostel) {
		...
		if (!$isInserted) {
			...
			throw new Exception(sprintf("Error inserting hostel (property number %s in db", $hostel["property_number"]));
		}
	...
	public function get_amenities_city_for_filter() {
		...
		return array("mostPopularAmenities" => $mostPopularAmenities, "amenities" => $amenities);
	}
	...
	function get_property_address($property_number = 0) {
		...
		if ($query -> num_rows() > 0) {
			$row = $query -> row();
			$address2 = (isset($row -> address2) AND $row -> address2 != NULL) ? ', ' . $row -> address2 : '';
			// address2 has value or null
	...
	public function get_property_districts_for_filter($property_number) {
		$this -> db -> select(self::HB_HOSTEL_DISTRICT_TABLE . ".district_id ," . self::DISTRICTS_TABLE . ".district_name ," . self::DISTRICTS_TABLE . ".um_id ");
	...
	private function getRatingsFromDbProperty($property) {
		$ratings = array("atmosphere" => round($property -> rating_atmosphere), "staff" => round($property -> rating_staff), "location" => round($property -> rating_location), "cleanliness" => round($property -> rating_cleanliness), "facilities" => round($property -> rating_facilities), "safety" => round($property -> rating_safety), "value" => round($property -> rating_value), );
