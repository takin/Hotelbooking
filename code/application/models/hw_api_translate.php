            ...
            foreach($properties['user_reviews'] as $r => $review)
            {
              if(!empty($gtrans["responseData"][$i]))
              {
                ...
                elseif(!empty($gtrans["responseData"][$i]))
                {
                  ...
                  log_message(TRANSLATION_ERROR_LEVEL,"translate_mixed_reviews: " .current_url()." -> [review] -> ".$properties_reviewed[$prop_number]['user_reviews'][$r]["review_translatedError"]. " | google status -> ".$gtrans["responseData"][$i]["responseStatus"]);
                }
                else
                {
                  $trans_error = TRUE;
