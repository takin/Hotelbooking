...
//  var $staging_apikey = "HostelBookers19367";
    ...
    /**
     * getPropertyRoomPricingWithBookingFee
     *
     * @param  propertyID	ID number of the property
     * @param  roomIds	String, list of room ids delimited by pipes (“|”)
     * @param  date	Date, arrival date in ‘dd-Mmm-yyyy’ format
     ...
     * @return
     */
    function getPropertyRoomPricingPerDateWithBookingFee($property_id, $roomsIDs, $startDate, $numNights, $language_code = "en", $strCurrencyCode = "") {
        try {
            $request_time = microtime(true);
            $return = $this->hbapi->getPropertyRoomPricingWithBookingFee($this->affiliate_name, $this->apikey, $property_id, $roomsIDs, $startDate, $numNights, $strCurrencyCode, $language_code);
            $response_time = microtime(true);
            $total_time = ($response_time - $request_time) * 1000;
            $total_time = floor($total_time);
            $total_time = $total_time . " ms ";
            $this->custom_log->log("audit", 'HB API getPropertyRoomPricingWithBookingFee ' . $total_time);
            return $return;
        } catch (SoapFault $exception) {
            log_message("Error", __FUNCTION__ . ' error: "' . $exception->faultcode . '" - ' . $exception->faultstring);
        }
     ...
     * @return
     */
    function make_booking($testmode = 0, $firstname, $lastname, $nationality, $male_count, $female_count, $tel, $email, $preferred_currency, $card_holder, $card_type, $card_number, $card_expiry_date, $card_cvv, $property_number, $dateStart, $arrival_time, $numNights, $room_selections, $beds_count, $language_code = "en", $card_issue = "", $card_start_Date = NULL, $comment = "") {
