	...
	<?php foreach ($city_landmarks as $city_landmark): ?>
		<li>
			...
			if( isset($filters['type']) ) {
				...
				switch ($filters['type']) {
					case 'campsite':
						echo anchor($this->uri->segment(1).'/'.$this->uri->segment(2).'/landmark/'.url_title($city_landmark -> landmark_name) .'/type/'. $category['campsite'] .(!empty($_SERVER['QUERY_STRING'])?'?'.$_SERVER['QUERY_STRING']:''), sprintf( gettext('Camping close to %s'), $city_landmark -> landmark_name), array('title' => sprintf( gettext('Camping close to %s'), $city_landmark -> landmark_name))); 
						break;
					
					case 'guesthouse':
						echo anchor($this->uri->segment(1).'/'.$this->uri->segment(2).'/landmark/'.url_title($city_landmark -> landmark_name).'/type/'. $category['guesthouse'] .(!empty($_SERVER['QUERY_STRING'])?'?'.$_SERVER['QUERY_STRING']:''), sprintf( gettext('Rooms close to %s'), $city_landmark -> landmark_name), array('title' => sprintf( gettext('Rooms close to %s'), $city_landmark -> landmark_name))); 
						break;
						
					case 'apartment':
						echo anchor($this->uri->segment(1).'/'.$this->uri->segment(2).'/landmark/'.url_title($city_landmark -> landmark_name).'/type/'. $category['apartment'] .(!empty($_SERVER['QUERY_STRING'])?'?'.$_SERVER['QUERY_STRING']:''), sprintf( gettext('Apartments close to %s'), $city_landmark -> landmark_name), array('title' => sprintf( gettext('Apartments close to %s'), $city_landmark -> landmark_name))); 
						break;
					
					case 'hotel':
						echo anchor($this->uri->segment(1).'/'.$this->uri->segment(2).'/landmark/'.url_title($city_landmark -> landmark_name).'/type/'. $category['hotel'] .(!empty($_SERVER['QUERY_STRING'])?'?'.$_SERVER['QUERY_STRING']:''), sprintf( gettext('Hotels close to %s'), $city_landmark -> landmark_name), array('title' => sprintf( gettext('Hotels close to %s'), $city_landmark -> landmark_name))); 
						break;
					
					case 'hostel':
						echo anchor($this->uri->segment(1).'/'.$this->uri->segment(2).'/landmark/'.url_title($city_landmark -> landmark_name).'/type/'. $category['hostel'] .(!empty($_SERVER['QUERY_STRING'])?'?'.$_SERVER['QUERY_STRING']:''), sprintf( gettext('Hostels close to %s'), $city_landmark -> landmark_name), array('title' => sprintf( gettext('Hostels in %s'), $city_landmark -> landmark_name))); 
						break;
					
					case 'property':
						echo anchor($this->uri->segment(1).'/'.$this->uri->segment(2).'/landmark/'.url_title($city_landmark -> landmark_name).'/type/'. $category['property'] .(!empty($_SERVER['QUERY_STRING'])?'?'.$_SERVER['QUERY_STRING']:''), sprintf( gettext('Properties close to %s'), $city_landmark -> landmark_name), array('title' => sprintf( gettext('Properties close to %s'), $city_landmark -> landmark_name))); 
						break;
