	...
	<?php foreach ($city_districts as $city_district): ?>
		<li>
			...
			if(isset($filters['type'])) {
				...
				switch ($filters['type']) {
					case 'campsite':
						echo anchor($this->uri->segment(1).'/'.$this->uri->segment(2).'/district/'.$city_district -> slug .'/type/'. $category['campsite'] . (!empty($_SERVER['QUERY_STRING'])?'?'.$_SERVER['QUERY_STRING']:''), sprintf( gettext('Camping in %s'), $city_district -> district_name), array('title' => sprintf( gettext('Camping in %s'), $city_district -> district_name))); 
						break;
					
					case 'guesthouse':
						echo anchor($this->uri->segment(1).'/'.$this->uri->segment(2).'/district/'.$city_district -> slug.'/type/'. $category['guesthouse'] . (!empty($_SERVER['QUERY_STRING'])?'?'.$_SERVER['QUERY_STRING']:''), sprintf( gettext('Rooms in %s'), $city_district -> district_name), array('title' => sprintf( gettext('Rooms in %s'), $city_district -> district_name))); 
						break;
						
					case 'apartment':
						echo anchor($this->uri->segment(1).'/'.$this->uri->segment(2).'/district/'.$city_district -> slug.'/type/'. $category['apartment'] . (!empty($_SERVER['QUERY_STRING'])?'?'.$_SERVER['QUERY_STRING']:''), sprintf( gettext('Apartments in %s'), $city_district -> district_name), array('title' => sprintf( gettext('Apartments in %s'), $city_district -> district_name))); 
						break;
					
					case 'hotel':
						echo anchor($this->uri->segment(1).'/'.$this->uri->segment(2).'/district/'.$city_district -> slug.'/type/'. $category['hotel'] . (!empty($_SERVER['QUERY_STRING'])?'?'.$_SERVER['QUERY_STRING']:''), sprintf( gettext('Hotels in %s'), $city_district -> district_name), array('title' => sprintf( gettext('Hotels in %s'), $city_district -> district_name))); 
						break;
					
					case 'hostel':
						echo anchor($this->uri->segment(1).'/'.$this->uri->segment(2).'/district/'.$city_district -> slug.'/type/'. $category['hostel'] . (!empty($_SERVER['QUERY_STRING'])?'?'.$_SERVER['QUERY_STRING']:''), sprintf( gettext('Hostels in %s'), $city_district -> district_name), array('title' => sprintf( gettext('Hostels in %s'), $city_district -> district_name))); 
						break;
					
					case 'property':
						echo anchor($this->uri->segment(1).'/'.$this->uri->segment(2).'/district/'.$city_district -> slug.'/type/'. $category['property'] . (!empty($_SERVER['QUERY_STRING'])?'?'.$_SERVER['QUERY_STRING']:''), sprintf( gettext('Properties in %s'), $city_district -> district_name), array('title' => sprintf( gettext('Properties in %s'), $city_district -> district_name))); 
						break;
