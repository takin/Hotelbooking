		...
		<li>
			...
			if(isset($filters['type'])) {
				...
				switch ($filters['type']) {
					...
					case 'property':
						echo anchor($this->uri->segment(1).'/'.$this->uri->segment(2).'/district/'.$city_district -> slug.'/type/'. $category['property'] . (!empty($_SERVER['QUERY_STRING'])?'?'.$_SERVER['QUERY_STRING']:''), sprintf( gettext('Properties in the %s district'), $city_district -> district_name), array('title' => sprintf( gettext('Properties in the %s district'), $city_district -> district_name))); 
						break;
