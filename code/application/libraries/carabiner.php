 ...
 *
 * @modified Louis-Michel Raynauld
 */
  ...
  public $script_dir  = '';
	...
	*/
	public function config($config)
	{
		...
		//FIX add trailing slash if not already there
		if(substr($this->base_uri, -1) != "/") $this->base_uri.= "/";
	...
	*/
	private function _display_js($group = 'main')
	{
		...
		// if we're combining files and minifying them
			...
			$filename = $lastmodified . md5($filenames.$this->https).'.js';
		...
		// if we're minifying. but not combining
			...
			foreach($this->js[$group] as $ref):
				...
				if( isset( $ref['prod']) ){
					...
					$f = filemtime( realpath( $this->script_path . $ref['dev'] ) ) . md5($ref['dev'].$this->https) . '.js';
		...
		if($this->dev){ // we're in a development environment
			...
			foreach($this->css[$group] as $media => $refs):
				...
				foreach ($refs as $ref):
					...
					//Fixed bug: _tag function was missing the cache parameter
					if(!$ref['combine'] && !$ref['minify']):
						echo (isset($ref['prod'])) ? $this->_tag('css', $ref['prod'], FALSE ,$media) : $this->_tag('css', $ref['dev'], FALSE, $media);
					elseif(!$ref['combine'] && $ref['minify']):
					  //Broken for now cannot minify if not combine should fix this later when needed
						echo (isset($ref['prod'])) ? $this->_tag('css', $ref['prod'], FALSE ,$media) : $this->_tag('css', $ref['dev'], FALSE, $media);
					elseif(!$ref['minify']):
						$files[] = (isset($ref['prod'])) ? array('prod'=>$ref['prod'], 'dev'=>$ref['dev'], 'minify'=>$ref['minify'] ) : array('dev'=>$ref['dev'], 'minify'=>$ref['minify']);
					else:
				...
				//Fixed bug: Do not create a combined file if files to combine array is empty
				if(!empty($files))
				{
  				$lastmodified = ($lastmodified == 0) ? '0000000000' : $lastmodified;
  				$filename = $lastmodified . md5($filenames.$this->https).'.css';
			...
			foreach($this->css[$group] as $media => $refs):
				...
				foreach($refs as $ref):
					...
					if( isset($ref['prod']) ){
						...
						$f = filemtime( realpath( $this->style_path . $ref['dev'] ) ) . md5($ref['dev'].$this->https) . '.css';
	...
	private function _tag($flag, $ref, $cache = FALSE, $media = 'screen')
	{
		...
		switch($flag){
			...
			case 'css':
				...
				return '<link type="text/css" rel="stylesheet" href="'.$dir.$ref.'" media="'.$media.'" charset="'.$this->CI->config->item('charset').'" />'."\r\n";
	...
	*/
	...
	public function load_group_assets($group_name)
	{
	  ...
	  {
	    $this->js['main'] = array_merge($this->js['main'],$this->js[$group_name]);
	  }

	  if(!empty($this->css[$group_name]))
	  {
	    $this->css['main'] = array_merge($this->css['main'],$this->css[$group_name]);
	  }
	}
}