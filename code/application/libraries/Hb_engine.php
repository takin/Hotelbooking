    ...
    function location_search($country, $city, $dateStart = NULL, $numNights = NULL, $include_availdata = FALSE, $prop_reviews = FALSE, $filters = array()) {
        ...
        if (empty($dateStart) && empty($numNights)) {
            ...
            $results = $this->CI->Db_hb_hostel->get_location_properties(
                    $city->country_system_name, $city->system_name, $this->api_functions_lang, $data['currency'], 25, $filters);
...
//            $json_data["property_list"][$i]['google_map_address'] = $address . ", " . $data["city_info"]->city_lname_en . ", " . $data["city_info"]->country_lname_en . ", " . $json_data["property_list"][$i]["zip"];
    ...
    $this->CI->Hb_api_translate->setLanguage($this->CI->site_lang, "en");
        ...
        if ($response === false) {
            ...
            // Get translation from HB feeds stored in the database
            if ($data['hostel']['LONGDESCRIPTION_translated'] == '') {
                if ($this->CI->db_hb_hostel->get_hb_long_desc($property_number, $this->CI->site_lang) != NULL) {
                    $data['hostel']['LONGDESCRIPTION_translated'] = $this->CI->db_hb_hostel->get_hb_long_desc($property_number, $this->CI->site_lang);
                }
            }
