...
class Hostelbookers_Property_Content_Service {
    ...
    private function getUrlsFromLanguageCodes($languageCodes) {
        ...
        foreach ($languageCodes as $langCode) {
            $url = sprintf("%s-[%s]-[%s]-[%s].xml", "http://feeds.hostelbookers.com/generic/PropertyContent", $langCode, date("Y"), date("m"));
    ...
    private function parseXmlData($xmlData, $langCode) {
        ...
        $endTime = microtime(true);
        $this->auditor->log("HB Property Content XML Service - Parsed monthly property content feed", $startTime, $endTime);
    ...
    private function insertOrUpdatePropertiesInDb($propertiesData) {
        ...
        $this->auditor->log("HB XML Service - Inserted/updated all property descriptions in DB", $startTime, $endTime);
    }
